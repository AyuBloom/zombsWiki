---
title: Module Overview - zombs.io Wiki
head:
  - - meta
    - property: 'og:title'
      content: Module Overview - zombs.io Wiki
  - - meta
    - name: description
      content: Script automatically generated by Emscripten during compilation of zombs_wasm.wasm. It serves as a JavaScript entry module instantiating and interacting with the WASM instance in BinCodec methods.
  - - meta
...
Wrapper around the WASM export `i`. The generated wrapper calls it after `__wasm_call_ctors()` has run. Calls imported function `_emscripten_run_script_int()` to set a random seed for the [Mersenne Twister](https://en.wikipedia.org/wiki/Mersenne_Twister) PRNG used during the PoW process.
...
| `_MakeBlendField(4, 152)` | Returns a pointer to the 64-byte PoW response used by `PACKET_ENTER_WORLD` or `PACKET_BLEND`. If the HUD check returns `0`, the response bytes are shuffled before `BinCodec` copies them. | `document.getElementById("hud").children.length;` |
...
The short names (`a`, `b`, `c`, etc.) are minified Emscripten import/export names. They are stable only for the analyzed generated file, not for every possible rebuild of `zombs_wasm.wasm`, as name shuffling has already happened once.
