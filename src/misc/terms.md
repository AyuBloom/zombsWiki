...
| SID | Server ID | Refers to the 5 character long ID that uniquely identifies a game server. (e.g. v4002, v3001) |
...
| AU | Auto Upgrade | Refers to a script function that automatically upgrades towers, once enough gold is available. |
| ATB | Auto Bow | Refers to a script function that automatically fires the player's weapon. |
| AA | Auto Aim | Refers to a script function that automatically aims the player's weapon at nearby targets. |
| AAZ | Auto Aim Zombies | Refers to a script function that specifically auto-aims at zombies, as opposed to other entities. |
| AAD | Auto Aim Demons | Refers to a script function that specifically auto-aims at demons, as opposed to other entities. |
| AWS | Auto Weapon Switch | Refers to a script function that automatically switches between weapons based on range. |
| PosLock | Position Lock | Refers to a script function that locks the player's position, preventing unintended movement. |
| TH | Tower Heal | Refers to a script function that activates the HealTowersSpell when a building reaches 20% of its health. |
| SlAll | Sell All | Refers to a script function that sells all placed buildings. |
| UpAll | Upgrade All | Refers to a script function that upgrades all buildings at once. |
| WASM | WebAssembly | Refers to the WebAssembly module that is a part of [`_MakeBlendField`](/mbf/overview). |
