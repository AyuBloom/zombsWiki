...
function checkFiles() {
  ...
  for (const filePath of mdFiles) {
    try {
      ...
      if (!hasTitle) {
        console.error(
          `Error: Missing 'title' in frontmatter for ${path.relative(process.cwd(), filePath)}`,
        );
        errorCount++;
        continue;
      }
      ...
      if (hasAnySeo && !hasAllSeo) {
        console.error(
          `Error: Incomplete SEO fields in 'head' for ${path.relative(process.cwd(), filePath)}. Expected either none, or all three (og:title, description, og:description).`,
        );
        errorCount++;
        continue;
      }
