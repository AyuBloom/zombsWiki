...
function extractTitleAndDescription(content) {
    ...
    const description = (foundParagraph && !isNonText)
        ? descriptionLines.join(' ').replace(/`/g, '').replace(/\[([^\]]+)\]\([^)]+\)/g, '$1')
        : null;
...
function updateHead(head, title, description) {
  ...
  if (description) {
        head.push(['meta', { property: 'og:title', content: `${title} - zombs.io Wiki` }]);
...
function processFiles() {
    ...
    for (const filePath of mdFiles) {
        try {
            ...
            if (!title) {
                console.log(`Skipped (No H1): ${path.relative(process.cwd(), filePath)}`);
                continue;
            }
            ...
            console.log(`Updated: ${path.relative(process.cwd(), filePath)}${description ? ' (with description)' : ' (no description)'}`);
        } catch (e) {
